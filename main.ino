...
bool at_right_T_junction = false;// can only go right or forwardF
...
const int buttonPin = !!!!!!!!!??????????; // 使用你选择的引脚  Choose the pin
volatile bool start = false; // 使用volatile关键字因为这个变量将在中断服务例程中被修改 Using volatile since it will be changed
...
void setup() {
  ...
  pinMode(buttonPin, INPUT_PULLUP); // 设置按钮引脚为输入并启用内部上拉电阻 set the pin as input
  attachInterrupt(digitalPinToInterrupt(buttonPin), buttonISR, FALLING); // 配置中断，对于按下按钮采用下降沿触发 set interupt


}
void loop() {
  while (!start) {
    delay(100); // 在start变为true之前循环等待 wait until start=true
  }
...
void buttonISR() {
  if (!start) {
    ...
    resetFunc(); // 如果按钮再次按下，重置程序 reset
    start = false; // 确保在程序重置后，start回到初始状态
  }
}

void(* resetFunc) (void) = 0; // 声明重置函数在地址0的位置 claim the position
